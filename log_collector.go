...
const (
	MaxLogReaderSize     = 10
	CheckpointFileExt    = ".cpt.json"
	OffsetDir            = "./offset"
	MaxLogCollectFailure = 5
	MaxIdleRound         = 10
)
...
type Checkpoint struct {
	...
	//TODO(link.xk): readers string   `json:"readers"`
}
...
// Watcher watched registerd log file event, triggering data collector when necessary. Design Doc: https://yuque.antfin-inc.com/link.xk/rvkkbv/nqxzpoy6skfx1sqo?singleDoc# 《新Log采集》
//
...
type LogCollector struct {
	...
	// fileNum  int          // num of collected file
...
func (lc *LogCollector) Init(conf *LogConf) error {
	...
	if err := lc.watcher.RegisterAndWatch(conf.Dir, conf.Pattern); err != nil {
		lc.Close()
	...
	log.Println("LogCollector init success")
	return nil
}
...
func (lc *LogCollector) makeCheckpoint(path string, meta *FileMeta, offset uint64) error {
	...
	// create a new file
	if err := ioutil.WriteFile(path, content, os.ModeAppend); err != nil {
...
func (lc *LogCollector) collectSchedule() {
	...
	for lc.running {
		...
		if err != nil {
			...
			// TODO(link.xk): check
			if lc.failedCounter[path] >= MaxLogCollectFailure {
		...
		if noProgress >= round {
			lc.idleCond.L.Lock()
			log.Printf("meet idle Cond, taskQ waitting, file %v\n", taskCtx.meta.path)
			lc.idleCond.Wait()
			log.Printf("event comes, taskQ signaledfile %v\n", taskCtx.meta.path)
			noProgress = 0
		...
		log.Printf("IfFullThenWait start %v", taskCtx.meta.path)
		lc.buf.IfFullThenWait()
		log.Printf("IfFullThenWait end %v", taskCtx.meta.path)
	}
}
